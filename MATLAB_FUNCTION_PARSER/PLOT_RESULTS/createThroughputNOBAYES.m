...
for gi=1:size(folds,1)
    ...
    if strcmp(folds(gi,1).name,'MNOBAYES5') % enter the nakagami folder
        pathDir=folds(gi,1).name;
        offM=1; % M=5 
    elseif strcmp(folds(gi,1).name,'MNOBAYES10')
        pathDir=folds(gi,1).name;
        offM=2; % M=10
        elseif strcmp(folds(gi,1).name,'MNOBAYES20')
            pathDir=folds(gi,1).name;
            offM=3; % M=20
            elseif strcmp(folds(gi,1).name,'MNOBAYES50')
                pathDir=folds(gi,1).name;
                offM=4; % M=50                       
                elseif strcmp(folds(gi,1).name,'MNOBAYES100')
                    pathDir=folds(gi,1).name;
                    offM=5; % M=100
%     end
    end
...
print(gcf,'-depsc' , 'chainMobNoBayes.eps');
% end
% swap =TPUT(4);
% TPUT(4)= TPUT(5)*1.2;
% TPUT(5)= swap;
