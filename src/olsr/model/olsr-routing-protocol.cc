...
RoutingProtocol::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::olsr::RoutingProtocol")
    ...
    .AddAttribute ("HoldHello", "HOLD time.", //MATTEO
                   TimeValue (Seconds (6)),
                   MakeTimeAccessor (&RoutingProtocol::m_holdHello),
                   MakeTimeChecker ())
    .AddAttribute ("HoldTc", "HOLD time.",
                   TimeValue (Seconds (15)),
                   MakeTimeAccessor (&RoutingProtocol::m_holdTc),
                   MakeTimeChecker ())
...
RoutingProtocol::RoutingProtocol ()
  : m_routingTableAssociation (0),
    m_holdHello(Seconds(6)),    
    m_holdTc(Seconds(15)),    
    m_helloInterval(Seconds(2)),
    m_tcInterval(Seconds(5)),
    m_ipv4 (0),
...
//MATTEO
Time RoutingProtocol::GetHelloInterval (void)
{
  return m_helloInterval;
}
//MATTEO
Time RoutingProtocol::GetTcInterval (void)
{
  return m_tcInterval;
}
...
RoutingProtocol::SendHello ()
{
  ...
  //QUI DA ERRORE DI ASSERT
  msg.SetVTime (m_holdHello); //MATTEO
          ...
          if (*neighIfaceAddr == receiverIface)
            {
              if (lt == OLSR_LOST_LINK)
              ...
              else if (lt == OLSR_SYM_LINK || lt == OLSR_ASYM_LINK)
                {
                  ...
                  link_tuple->time = link_tuple->symTime + m_holdHello;
                  updated = true;
                }
              else
                {
...
void RoutingProtocol::HelloTimerStop ()
{
  m_helloTimer.Cancel();
  //m_holdHello=Seconds(1);
  SendHello ();
  m_helloTimer.Schedule (m_helloInterval);
}

void RoutingProtocol::TcTimerStop ()
{
  m_tcTimer.Cancel();
  //m_holdTc=Seconds(2);
  if (m_state.GetMprSelectors ().size () > 0)
    {
    //NS_LOG_DEBUG("at time " << Simulator::Now().GetSeconds() << " send TC");
      SendTc ();
    }
  else
    {
