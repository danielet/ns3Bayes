...
namespace ns3 {
std::map<int , FILE *> TcpNewReno::FILE_DIC;
...
TcpNewReno::NewAck (const SequenceNumber32& seq)
{
  ...
  NS_LOG_LOGIC ("TcpNewReno receieved ACK for seq " << seq <<
    ...
    if(FILE_DIC.find(m_node->GetId ()) == FILE_DIC.end()){
      FILE* log_file;
      char * fname;
      fname = (char*)malloc(sizeof(char) * 255);  
      memset(fname, 0, sizeof(char) * 255);    
      sprintf(fname, "CongestionWindow_node_%d.txt", m_node->GetId ());
      log_file = fopen(fname, "w+");
      FILE_DIC[m_node->GetId ()] = log_file;
      // printf("ID NODE %d \n" , m_node->GetId ());
    }
    else
    {
      //ESTRARRE IL FILE CORRETTO DOVE ANDARE A SCRIVERE
      FILE * a = FILE_DIC.at(m_node->GetId ());
      uint32_t cwnd = static_cast<uint32_t> (m_cWnd); //m_cWnd.GetValue();
      fprintf(a, "%f\t %i\n", Simulator::Now().GetSeconds(), cwnd);
      fflush(a);
    }
