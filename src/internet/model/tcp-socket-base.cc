...
// uint32_t TcpSocketBase::firstReceiveAck=1 ;
// uint32_t TcpSocketBase::firstSendPacket=1 ;
// FILE* TcpSocketBase::log_fileSDPacket;
// FILE* TcpSocketBase::log_fileRCAck;

std::map<int , FILE *> TcpSocketBase::FILE_DIC_SDPacket;
std::map<int , FILE *> TcpSocketBase::FILE_DIC_RCPacket;
...
TcpSocketBase::~TcpSocketBase (void)
{
  ...
  m_tcp = 0;
  m_bayes = 0; //MATTEO
  printf("DISTRUCTOR TCP SOCKET BASE \n");
  // fclose(log_fileRCAck);
  // fclose(log_fileSDPacket);
...
void //(MATTEO)
TcpSocketBase::AddBayes(Ptr<Bayes> bayes)
{
  m_bayes = bayes;
}
  
void //(MATTEO)
TcpSocketBase::CheckBayes(void)
{
  NS_LOG_DEBUG ("At time " << Simulator::Now ().GetSeconds () << " check bayes: " << m_bayes->m_corruptedIPtables);
  if (m_bayes->m_corruptedIPtables) {
    //NS_LOG_DEBUG ("At time " << Simulator::Now ().GetSeconds () << " try check bayes: " << m_bayes->m_corruptedIPtables);
    Simulator::Schedule (Seconds(0.1), &TcpSocketBase::CheckBayes, this);
  }
  else {
    m_rto = Seconds (1.0); // restart! 
    NS_LOG_DEBUG (this << " SendDataPacket Schedule ReTxTimeout at time after BAYES CHECK! " <<
...
TcpSocketBase::ReceivedAck (Ptr<Packet> packet, const TcpHeader& tcpHeader)
{
  NS_LOG_FUNCTION (this << tcpHeader);
    ...
    uint32_t acked = tcpHeader.GetAckNumber ().GetValue ()-1;


  

  if(FILE_DIC_RCPacket.find(m_node->GetId ()) == FILE_DIC_RCPacket.end())
  {

    char* fname = (char*)malloc(sizeof(char) * 255);  
    memset(fname, 0, sizeof(char) * 255);
    sprintf(fname, "ACK_received_by_node_%d.txt", m_node->GetId ());
    FILE* log_file;
    log_file = fopen(fname, "w+");
    FILE_DIC_RCPacket[m_node->GetId ()] = log_file;
    if(fname)
      free(fname);
    fprintf(log_file, "%f\t %i\n", Simulator::Now().GetSeconds(), acked);
    fflush(log_file);
  }
  else
  {
    FILE* log_file = FILE_DIC_RCPacket.at(m_node->GetId ());
    fprintf(log_file, "%f\t %i\n", Simulator::Now().GetSeconds(), acked);
    fflush(log_file);
  }
...
TcpSocketBase::SendDataPacket (SequenceNumber32 seq, uint32_t maxSize, bool withAck)
{
  ...
  uint32_t lastAck_received = m_txBuffer.HeadSequence ().GetValue();
