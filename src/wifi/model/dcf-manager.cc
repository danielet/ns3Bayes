...
std::map<int , FILE *> DcfManager::FILE_DIC;
/****************************************************************
...
DcfManager::RequestAccess (DcfState *state)
{
  ...
  //MATTEO collection
  Ptr<YansWifiPhy> phys = DynamicCast<YansWifiPhy> (m_phy);
  Ptr<Object> srcNetDevice = phys->GetDevice ();

//MATTEO collection
  uint32_t srcNode = srcNetDevice->GetObject<NetDevice> ()->GetNode ()->GetId ();
  if(FILE_DIC.find(srcNode) == FILE_DIC.end())
  {
    FILE* log_file;
    char* fname = (char*)malloc(sizeof(char) * 255);  
    memset(fname, 0, sizeof(char) * 255);
    sprintf(fname, "ContentionWindow_node_%d.txt", srcNode);
    log_file = fopen(fname, "w+");
    FILE_DIC[srcNode] = log_file;
    if(fname)
      free(fname);
    fprintf(log_file, "%f\t %i\n", Simulator::Now().GetSeconds(), (int)state->GetCw());
    fflush(log_file);
  }
  else
  {
    FILE * log_file = FILE_DIC.at(srcNode);
    fprintf(log_file, "%f\t %i\n", Simulator::Now().GetSeconds(), (int)state->GetCw());
    fflush(log_file);
  }
