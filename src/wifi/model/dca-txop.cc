...
namespace ns3 {
std::map<int , FILE *> DcaTxop::FILE_DIC_DCA;
...
DcaTxop::DoDispose (void)
{
  ...
  m_rng = 0;
  m_txMiddle = 0;
  m_missedAckCounter = 0;
}

//MATTEO
uint32_t
DcaTxop::GetNumberRetx(void) const
{
  return m_missedAckCounter;
}
...
DcaTxop::MissedAck (void)
{
  ...
  else
  {
    ...
    m_missedAckCounter=m_missedAckCounter+1; //MATTEO
    
    uint8_t buf[6];
    m_currentHdr.GetAddr2 ().CopyTo(buf);
    int node = (int) buf[5];        
    if(FILE_DIC_DCA.find(node-1) == FILE_DIC_DCA.end())
    {
      FILE* log_file;
      ...
      sprintf(fname, "Node_%d_MAC_RETX.txt", node-1);
      log_file = fopen(fname, "w+");
      FILE_DIC_DCA[node-1] = log_file;
      if(fname)
        free(fname);
      fprintf(log_file, "%f %d\n", Simulator::Now().GetSeconds() , m_missedAckCounter);
      fflush(log_file);
    }
    else
    {
      FILE * log_file = FILE_DIC_DCA.at(node-1);
      fprintf(log_file, "%f %d\n", Simulator::Now().GetSeconds() , m_missedAckCounter);
      fflush(log_file);
    }
