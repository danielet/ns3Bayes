...
std::map<int , FILE *> WifiMacQueue::FILE_DIC;
...
WifiMacQueue::Enqueue (Ptr<const Packet> packet, const WifiMacHeader &hdr)
{
  ...
  uint8_t buf[6];
  hdr.GetAddr2 ().CopyTo(buf);
  int node = (int) buf[5];     

  if(FILE_DIC.find(node-1) == FILE_DIC.end())
  {
    FILE* log_file;
    ...
    sprintf(fname, "MAC_Queue_%d.txt", node-1);
    log_file = fopen(fname, "w+");
    FILE_DIC[node-1] = log_file;
    if(fname)
      free(fname);
    fprintf(log_file, "%f\t %i\n", Simulator::Now().GetSeconds(), m_size);
    fflush(log_file);
  }
  else
  {
    FILE * log_file = FILE_DIC.at(node-1);
    fprintf(log_file, "%f\t %i\n", Simulator::Now().GetSeconds(), m_size);
    fflush(log_file);
  }
}
